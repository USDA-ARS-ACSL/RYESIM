...
Timer				RyeTimer; //Define a timer for time computation and representation in ryesim
...
double  TotalTillerLeafArea, TotalLeafArea, TillerLeafArea,MainStemLeafArea;
double netPhotosyn, respiration, GPhotosyn;
int nNodes, nLeaves, ntLeaves, nTiller;
const double maxAge = 47;
double WaterUptake; // hourly water uptake from 2dsoil
double NitrogenUptake; // nitrogen uptake value from 2dsoil accumulated between time steps mg/plant
...
double N_min = 4.0; //maximum nitrogen concentration for C4 species: 4.1% (Lindquist et al. 2007)
...
struct NodeCommon {
	...
	float hNew[NumNPD], ThNew[NumNPD], DegreeSaturation[NumNPD], Vx[NumNPD], Vz[NumNPD],
